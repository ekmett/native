...
import native;
using B = native::simd<native::fp16,8,native::neon_fp16>;
...
static_assert(std::same_as<decltype(native::simd<native::fp16,8,native::neon_fp16>(std::array<native::fp16,8>{})),B>);
...
template<std::size_t... I> auto lane_construction(std::index_sequence<I...>) {
  return native::simd<native::fp16,sizeof...(I),native::neon_fp16>(native::fp16::from_bits(std::uint16_t(I))...);
}
static_assert(std::same_as<decltype(lane_construction(std::make_index_sequence<8>{})),B>);

extern "C" bool fp16_storage() {
  std::array<native::fp16,8> values, copy;
  ...
  for (unsigned base=0; base!=65536; base+=8) {
    for (unsigned i=0;i!=8;++i) values[i]=native::fp16::from_bits(std::uint16_t(base+i));
