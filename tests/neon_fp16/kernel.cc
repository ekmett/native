...
import simd;
using B = simd::vec<simd::fp16,8,simd::neon_fp16>;
...
static_assert(std::same_as<decltype(simd::vec<simd::fp16,8,simd::neon_fp16>(std::array<simd::fp16,8>{})),B>);
...
template<std::size_t... I> auto lane_construction(std::index_sequence<I...>) {
  return simd::vec<simd::fp16,sizeof...(I),simd::neon_fp16>(simd::fp16::from_bits(std::uint16_t(I))...);
}
static_assert(std::same_as<decltype(lane_construction(std::make_index_sequence<8>{})),B>);

extern "C" bool fp16_storage() {
  std::array<simd::fp16,8> values, copy;
  ...
  for (unsigned base=0; base!=65536; base+=8) {
    for (unsigned i=0;i!=8;++i) values[i]=simd::fp16::from_bits(std::uint16_t(base+i));
  ...
  for (std::size_t n=0;n<=8;++n) {
    auto * input=reinterpret_cast<simd::fp16 *>(input_page.end())-n;
    auto * output=reinterpret_cast<simd::fp16 *>(output_page.end())-n;
    for (std::size_t i=0;i<n;++i) {
      std::construct_at(input+i,simd::fp16::from_bits(std::uint16_t(0x7c00+i)));
      std::construct_at(output+i,simd::fp16::from_bits(0xdead));
    }
    std::construct_at(output-1,simd::fp16::from_bits(0xbeef));
    auto value=B::load_partial(input,n,simd::fp16::from_bits(0x8001));
    ...
    auto generic=simd::load_simd_partial<B>(input,n,simd::fp16::from_bits(0x8001));
  ...
  B::load_partial(nullptr,0,simd::fp16::from_bits(0x7c01)).store(copy.data());
  for (auto value:copy) if(value.to_bits()!=0x7c01) return false;
  B{}.store_partial(nullptr,0);
  B(simd::fp16::from_bits(0xfc01)).store(copy.data());
...
extern "C" simd_noinline void fp16_add(std::uint16_t const *a,std::uint16_t const *b,std::uint16_t *out) noexcept {
  (B::load_bits(a)+B::load_bits(b)).store_bits(out);
}
extern "C" simd_noinline void fp16_sub(std::uint16_t const *a,std::uint16_t const *b,std::uint16_t *out) noexcept {
  (B::load_bits(a)-B::load_bits(b)).store_bits(out);
}
...
extern "C" simd_noinline void fp16_neg(std::uint16_t const *a,std::uint16_t *out) noexcept {
  (-B::load_bits(a)).store_bits(out);
}
extern "C" simd_noinline void fp16_compare(std::uint16_t const *a,std::uint16_t const *b,std::uint64_t *out) noexcept {
...
extern "C" simd_noinline void fp16_select(std::uint64_t mask,std::uint16_t const *a,std::uint16_t const *b,std::uint16_t *out) noexcept {
