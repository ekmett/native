...
import native;
using B = native::vec<native::fp16,8,native::neon_fp16>;
...
static_assert(std::same_as<decltype(native::vec<native::fp16,8,native::neon_fp16>(std::array<native::fp16,8>{})),B>);
...
template<std::size_t... I> auto lane_construction(std::index_sequence<I...>) {
  return native::vec<native::fp16,sizeof...(I),native::neon_fp16>(native::fp16::from_bits(std::uint16_t(I))...);
}
