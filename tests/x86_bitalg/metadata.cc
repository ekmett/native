...
namespace {
  ...
  constexpr auto full_width = native::target_features<native::x86>("avx512bitalg");
  constexpr auto short_width = native::target_features<native::x86>("avx512bitalg,avx512vl");
  ...
  static_assert(!native::avx512.has(x86_feature::avx512bitalg));
  ...
  static_assert(native::x86_feature_count > 38);
  static_assert(full_width == native::feature_closure(x86_feature::avx512bitalg));
  static_assert(native::target_features<native::x86>("no-avx512bitalg") ==
    native::detail::invalid_features<native::x86>);
  static_assert(native::target_features<native::x86>("avx512bitalg,") ==
    native::detail::invalid_features<native::x86>);

#define NATIVE_TARGET_metadata_bitalg "avx512bitalg"
  static_assert(NATIVE_TARGET_ISA(metadata_bitalg) == full_width);
#if NATIVE_TEST_BITALG_MINIMUM
  static_assert(NATIVE_BASELINE.has(x86_feature::avx512bitalg));
...
#else
  static_assert(!NATIVE_BASELINE.has(x86_feature::avx512bitalg));
  static_assert(!NATIVE_TARGET_MINIMUM.has(x86_feature::avx512bitalg));
#endif

  constexpr bool properties() {
    native::isa<native::x86> features = x86_feature::popcnt;
    features.avx512bitalg = true;
    if (!features.avx512bitalg || features != (x86_feature::popcnt & x86_feature::avx512bitalg)) {
      return false;
    }
  ...
  constexpr bool normalized_admission() {
    for (auto requirements : {full_width, short_width}) {
      ...
      for (auto feature : {x86_feature::avx512f, x86_feature::avx512bw, x86_feature::avx512bitalg, x86_feature::avx512vl}) {
  ...
  // Independent raw CPUID words exercise AVX512BITALG at leaf 7, subleaf 0, ECX12.
  struct raw_snapshot {
  ...
  constexpr bool raw_admission() {
    ...
    for (unsigned bit = 0; bit < 32; ++bit) {
      ...
      if (!decoded.observed.has(x86_feature::avx512bitalg) ||
          decoded.present.has(x86_feature::avx512bitalg) != (bit == 12) ||
          native::classify_isa(cpu, full_width).admitted() != (bit == 12)) {
        return false;
      }
    }
    raw_snapshot cpu;
    ...
    for (auto maximum : {0u, 1u, 6u}) {
      ...
      auto decoded = native::detail::decode_x86_features(cpu);
      if (decoded.observed.has(x86_feature::avx512bitalg) ||
          decoded.present.has(x86_feature::avx512bitalg) ||
...
int main() {
  ...
  std::puts("AVX512BITALG feature and OS-state admission checks passed.");
  return 0;
}
