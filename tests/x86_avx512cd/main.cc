...
int main(int argc, char **) {
  ...
  if (native_avx512cd_baseline_import(probe) != ((probe >> 3) ^ (probe + 17))) {
    return 1;
  }
  auto cpu = native::observe_x86_capabilities();
  if (!native::classify_isa(cpu, cd512).admitted()) {
    std::puts(
        "SKIP AVX512CD: CPU features or OS ZMM state unavailable; constexpr/shape checks passed.");
    return 77;
  }
  std::uint64_t state = 0x9e3779b97f4a7c15ull;
  if (!check_vectors<std::uint32_t, 16>(state) || !check_vectors<std::uint64_t, 8>(state)) {
    return 2;
  }
  if (native::classify_isa(cpu, cdvl).admitted()) {
    if (!check_vectors<std::uint32_t, 4>(state) || !check_vectors<std::uint32_t, 8>(state) ||
        !check_vectors<std::uint64_t, 2>(state) || !check_vectors<std::uint64_t, 4>(state)) {
      return 3;
    }
  } else {
