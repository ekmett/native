...
#define RDM_CONTRACT(name) \
  ...
  template<isa Arch, class T> concept explicit_##name = requires(T x) { native::name<Arch>(x,x,x); }; \
  template<class T> consteval bool name##_contract() { \
    return default_##name<T,T,T> == present && explicit_##name<rdm,T> \
      && !explicit_##name<isa(arm_feature::neon),T>; \
  } \
...
using bytes = native::simd<std::uint8_t,8,isa(arm_feature::neon)>;
...
static_assert(std::same_as<decltype(native::pmull(bytes{},bytes{})),native::simd<std::uint16_t,8,isa(arm_feature::neon)>>);
