...
namespace {
  ...
  constexpr auto full_width = native::target_features<native::x86>("avx512vbmi2");
  constexpr auto short_width = native::target_features<native::x86>("avx512vbmi2,avx512vl");
  ...
  static_assert(!native::avx512.has(x86_feature::avx512vbmi2));
  ...
  static_assert(native::x86_feature_count > 40);
  static_assert(full_width == native::feature_closure(x86_feature::avx512vbmi2));
  static_assert(native::target_features<native::x86>("no-avx512vbmi2") ==
    native::detail::invalid_features<native::x86>);
  static_assert(native::target_features<native::x86>("avx512vbmi2,") ==
    native::detail::invalid_features<native::x86>);

#define NATIVE_TARGET_metadata_vbmi2 "avx512vbmi2"
  static_assert(NATIVE_TARGET_ISA(metadata_vbmi2) == full_width);
#if NATIVE_TEST_VBMI2_MINIMUM
  static_assert(NATIVE_BASELINE.has(x86_feature::avx512vbmi2));
...
#else
  static_assert(!NATIVE_BASELINE.has(x86_feature::avx512vbmi2));
  static_assert(!NATIVE_TARGET_MINIMUM.has(x86_feature::avx512vbmi2));
#endif

  constexpr bool properties() {
    native::isa<native::x86> features = x86_feature::popcnt;
    features.avx512vbmi2 = true;
    if (!features.avx512vbmi2 || features != (x86_feature::popcnt & x86_feature::avx512vbmi2)) {
      return false;
    }
  ...
  constexpr bool normalized_admission() {
    for (auto requirements : {full_width, short_width}) {
      ...
      for (auto feature : {x86_feature::avx512f, x86_feature::avx512bw,
                           x86_feature::avx512vbmi2, x86_feature::avx512vl}) {
  ...
  // Independent raw CPUID words exercise AVX512VBMI2 at leaf 7, subleaf 0, ECX6.
  struct raw_snapshot {
  ...
  constexpr bool raw_admission() {
    ...
    for (unsigned bit = 0; bit < 32; ++bit) {
      ...
      if (!decoded.observed.has(x86_feature::avx512vbmi2) ||
          decoded.present.has(x86_feature::avx512vbmi2) != (bit == 6) ||
          native::classify_isa(cpu, full_width).admitted() != (bit == 6)) {
        return false;
      }
    }
    raw_snapshot cpu;
    ...
    for (auto maximum : {0u, 1u, 6u}) {
      ...
      auto decoded = native::detail::decode_x86_features(cpu);
      if (decoded.observed.has(x86_feature::avx512vbmi2) ||
          decoded.present.has(x86_feature::avx512vbmi2) ||
...
int main() {
  ...
  std::puts("AVX512VBMI2 feature and OS-state admission checks passed.");
  return 0;
}
