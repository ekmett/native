...
static_assert(std::same_as<decltype(simd::vec<simd::fp16,32,simd::avx512_fp16>(std::array<simd::fp16,32>{})),B>);
...
template<std::size_t... I> auto lane_construction(std::index_sequence<I...>) {
  return simd::vec<simd::fp16,sizeof...(I),simd::avx512_fp16>(simd::fp16::from_bits(std::uint16_t(I))...);
}
static_assert(std::same_as<decltype(lane_construction(std::make_index_sequence<32>{})),B>);

extern "C" bool fp16_storage() {
  std::array<simd::fp16,32> values, copy;
  ...
  for (unsigned base=0; base!=65536; base+=32) {
    for (unsigned i=0;i!=32;++i) values[i]=simd::fp16::from_bits(std::uint16_t(base+i));
