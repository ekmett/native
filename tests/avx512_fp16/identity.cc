...
using instruction_half = native::simd<native::fp16,4,native::avx512_fp16>;
static_assert(complete<instruction_half>);
static_assert(!addable<instruction_half,instruction_half>);
static_assert(!complete<native::simd<native::bf16,32,native::avx512_fp16>>);
template<class T> concept divisible=requires(T a) { a/a; };
static_assert(!divisible<instruction_half>);
