...
boundary. BW is still a backend attribute obligation. Binary32 result construction
now follows its raw float implementation requirements; extra BF16/FP16 bits in
the retained caller Arch do not require half-feature attributes. Native half
elements retain their separate implementation requirements.
...
The production refinement composes the five x86 raw-exp backend scopes from
`src/simd/simd/for_each_backend.h` with the five raw binary32-result constructor
scopes. Their 25 tuples produce five cells. This narrowing relies on the builtin
float constructors and memory helpers using their actual implementation
requirements rather than every feature in their caller tag. The internal policy
header has a complete source-name and literal-target record for those cells;
static assertions compare every record with the computed union and both original ordinals. C++ does not convert the
...
shape. Lane count, register count, and Flush remain independent template
parameters. BW remains in the current raw backend cover. Scalar, custom ADL and
ARM extension paths retain the existing generic wide implementation. Runtime checks cover a tagged array customization, an untagged
element fallback, scalar values and conditional noexcept. Flush=true must not
accept an untagged-only customization.

The native test retains eleven caller tags independently of the five production
cells, including BF16, FP16 and both-half combinations with and without VL. Its
historical stronger caller scopes remain covered. A second evaluator set compiles
those same caller types under the five raw scopes, with no half attributes, and
uses the selected raw requirements for CPU/OS admission. Only admitted entries
execute. Separate non-inline production/reference entry points compare exact outputs for
...
The same native object retains all 22 original code-generation entries and the
unchanged strict checker, comparing eight-lane and native-width forms. The checker requires packed FMA, native SCALEF exactly
...
It also compiles both-half caller tags under public raw BW and full/VL target
scopes, checking the exported overload and result type without private metadata.
