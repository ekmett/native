...
// Experimental fixture only. This deliberately reuses internal entry validation
// and host masks; no new public refinement API is exported by simd.
namespace refinement_test {
  template<class List> struct entries;
  template<class... E> struct entries<simd::isa_list<E...>> {
    static constexpr std::array<simd::feature_set,sizeof...(E)> requirements{
      simd::detail::abi_match<E,0>::required_features...};
  };

  template<class... Lists> struct refinement {
    static_assert(sizeof...(Lists)>0);
    static constexpr std::size_t dimensions=sizeof...(Lists);
    static constexpr std::size_t capacity=(std::size_t{1} * ... * entries<Lists>::requirements.size());
    struct cell {
    ...
    template<std::size_t K> static consteval auto choices() {
      constexpr std::array radices{entries<Lists>::requirements.size()...};
    ...
    static consteval cell candidate(std::index_sequence<I...>) {
      ...
      constexpr auto requirements=simd::feature_closure(
        (simd::feature_set{0} | ... | entries<Lists>::requirements[selected[I]]));
