...
concept has_sdot = requires(R r, X x) {
  { native::sdot<A>(r, x, x) } noexcept -> std::same_as<R>;
};
template<native::isa A, int Lane, class R, class X, class Y>
concept has_sdot_lane = requires(R r, X x, Y y) {
  { native::sdot_lane<A, Lane>(r, x, y) } noexcept -> std::same_as<R>;
};
template<native::isa A, class R, class X>
concept has_udot = requires(R r, X x) {
  { native::udot<A>(r, x, x) } noexcept -> std::same_as<R>;
};
template<native::isa A, int Lane, class R, class X, class Y>
concept has_udot_lane = requires(R r, X x, Y y) {
  { native::udot_lane<A, Lane>(r, x, y) } noexcept -> std::same_as<R>;
};
...
bool check_sdot_int32x2(std::uint64_t & state) {
  for (unsigned iteration = 0; iteration < 2048; ++iteration) {
    ...
    auto result = std::bit_cast<std::array<std::int32_t, 2>>(native::sdot<requirements>(av, bv, cv));
    ...
    }
    {
      ...
      [&]<std::size_t... Lane>(std::index_sequence<Lane...>) __attribute__((target("dotprod"))) {
        auto check = [&]<std::size_t L>() __attribute__((target("dotprod"))) {
          auto result = std::bit_cast<std::array<std::int32_t, 2>>(native::sdot_lane<requirements, L>(av, bv, cv));
          for (unsigned i = 0; i < 2; ++i) {
            auto expected = static_cast<std::uint32_t>(a[i]);
            for (unsigned j = 0; j < 4; ++j)
...
bool check_sdot_int32x4(std::uint64_t & state) {
  for (unsigned iteration = 0; iteration < 2048; ++iteration) {
    ...
    auto cv = std::bit_cast<int8x16_t>(c);
    auto result = std::bit_cast<std::array<std::int32_t, 4>>(native::sdot<requirements>(av, bv, cv));
    ...
    }
    {
      ...
      [&]<std::size_t... Lane>(std::index_sequence<Lane...>) __attribute__((target("dotprod"))) {
        auto check = [&]<std::size_t L>() __attribute__((target("dotprod"))) {
          auto result = std::bit_cast<std::array<std::int32_t, 4>>(native::sdot_lane<requirements, L>(av, bv, cv));
          for (unsigned i = 0; i < 4; ++i) {
            auto expected = static_cast<std::uint32_t>(a[i]);
            for (unsigned j = 0; j < 4; ++j)
              expected += static_cast<std::uint32_t>(static_cast<std::int32_t>(b[4*i+j]) * c[4*L+j]);
...
bool check_udot_uint32x2(std::uint64_t & state) {
  for (unsigned iteration = 0; iteration < 2048; ++iteration) {
    ...
    auto result = std::bit_cast<std::array<std::uint32_t, 2>>(native::udot<requirements>(av, bv, cv));
    for (unsigned i = 0; i < 2; ++i) {
      auto expected = static_cast<std::uint32_t>(a[i]);
      for (unsigned j = 0; j < 4; ++j)
        expected += static_cast<std::uint32_t>(static_cast<std::int32_t>(b[4*i+j]) * c[4*i+j]);
    ...
    }
    {
      ...
      [&]<std::size_t... Lane>(std::index_sequence<Lane...>) __attribute__((target("dotprod"))) {
        auto check = [&]<std::size_t L>() __attribute__((target("dotprod"))) {
          auto result = std::bit_cast<std::array<std::uint32_t, 2>>(native::udot_lane<requirements, L>(av, bv, cv));
          for (unsigned i = 0; i < 2; ++i) {
            auto expected = static_cast<std::uint32_t>(a[i]);
            for (unsigned j = 0; j < 4; ++j)
              expected += static_cast<std::uint32_t>(static_cast<std::int32_t>(b[4*i+j]) * c[4*L+j]);
...
bool check_udot_uint32x4(std::uint64_t & state) {
  for (unsigned iteration = 0; iteration < 2048; ++iteration) {
    ...
    auto cv = std::bit_cast<uint8x16_t>(c);
    auto result = std::bit_cast<std::array<std::uint32_t, 4>>(native::udot<requirements>(av, bv, cv));
    for (unsigned i = 0; i < 4; ++i) {
      auto expected = static_cast<std::uint32_t>(a[i]);
      for (unsigned j = 0; j < 4; ++j)
        expected += static_cast<std::uint32_t>(static_cast<std::int32_t>(b[4*i+j]) * c[4*i+j]);
    ...
    }
    {
      ...
      [&]<std::size_t... Lane>(std::index_sequence<Lane...>) __attribute__((target("dotprod"))) {
        auto check = [&]<std::size_t L>() __attribute__((target("dotprod"))) {
          auto result = std::bit_cast<std::array<std::uint32_t, 4>>(native::udot_lane<requirements, L>(av, bv, cv));
          for (unsigned i = 0; i < 4; ++i) {
            auto expected = static_cast<std::uint32_t>(a[i]);
            for (unsigned j = 0; j < 4; ++j)
              expected += static_cast<std::uint32_t>(static_cast<std::int32_t>(b[4*i+j]) * c[4*L+j]);
