...
"""Check every DOTPROD wrapper at a baseline AArch64 source minimum."""
import argparse
...
for name in names:
    ...
    if "_lane" in name:
        lane = int(name.rsplit("_lane", 1)[1]) // 4 - 1
    ...
    if re.search(r"\b(?:sqrdmlah|sqrdmlsh|fmlal|fmlal2|fcmla|bfmmla|bfdot)\b", body):
        raise SystemExit(f"{name} requires an unrelated instruction family")
print(f"{len(names)} DOTPROD fixtures contain their native instructions without helper calls.")
