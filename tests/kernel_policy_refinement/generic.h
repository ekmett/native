...
#undef ARRAY
  ...
  static_assert(noexcept(abs(native::wide<value,3>{})));
  static_assert(!noexcept(abs(native::wide<throwing,3>{})));
  static_assert(!std::is_nothrow_constructible_v<native::wide<throwing_conversion,3>,native::wide<value,3> const &>);
  bool check() {
    using W=native::wide<value,3>;
    W a{value{-1},value{2},value{-3}};
    arrays=elements=0;
    auto b=a+a;
    auto c=abs(a);
    if(arrays!=2 || elements!=0 || b.registers[2].word!=-6 || c.registers[0].word!=1) return false;
    native::wide<converted,3> d{a};
    if(d.registers[2].word!=-3) return false;
    native::wide<value,0> empty{};
    if(empty.apply([]{return 17;})!=17) return false;
    auto e=abs(empty);
    if(!e.registers.empty() || arrays!=3 || elements!=0) return false;
#define CHECK(name) {auto before=arrays;auto result=native::name(a); \
...
#undef CHECK
#define CHECK_ELEMENT(name) {auto before=arrays;auto count=elements;auto result=native::name(a); \
...
#undef CHECK_ELEMENT
    elements=0;
    auto before=arrays;
    auto angle=native::atan2(a,a);auto fused=native::fma(a,a,a);auto pair=native::sincos(a);
    ...
    before=elements;
    auto finite=native::isfinite(a);auto infinite=native::isinf(a);
    auto nan=native::isnan(a);auto sign=native::signbit(a);
    auto signed_copy=native::copysign(c,a);
    ...
    auto scalar=native::sqrt(native::wide<float,2>{4.f,9.f});
    if(scalar.registers[0]!=2.f || scalar.registers[1]!=3.f) return false;
#if defined(__cpp_exceptions) || defined(_CPPUNWIND)
    cleanups=0;
    try { auto unused=abs(native::wide<throwing,1>{throwing{4}});(void)unused;return false; }
    catch(failure const &) {}
