...
FDQ_EVEX = {
    ...
    'vpmovqd', 'vpmovdw', 'vinserti64x4', 'vshufi64x2'}
...
def check_body(name, text):
    ...
    if name.endswith(('_reinterpret', '_identity')):
        if any(m[2].startswith(('call', 'j', 'loop')) or m[2] == '<unknown>' for m in instructions):
            raise ValueError('Outlined or undecoded identity boundary: ' + name)
        return 'register identity boundary'
    ...
    if no_bw:
        expected = 'vpmovdw' if name.endswith('pack_words') else 'vpmovqd' if name.endswith('pack') else 'vpsrld' if name.endswith('u32') else 'vpsrlq'
...
def check_identity_pair(identity, reinterpreted):
    def body(text):
        return [(m[2], m[3].split('#')[0].strip()) for m in INSTRUCTION.finditer(text)
                if not m[2].startswith('nop')]
    if body(identity) != body(reinterpreted):
        raise ValueError('Bit reinterpretation emitted work beyond its raw identity boundary')
...
def controls():
    ...
    identity_name = 'integer_codegen_fdq_reinterpret'
    identity = f'0000000000000000 <{identity_name}>:\n 0: c3 retq\n'
    check_body(identity_name, identity)
    try:
        check_identity_pair(identity, identity.replace(' 0: c3', ' 0: 48 89 c0 movq %rax, %rax\n 3: c3'))
    except ValueError:
        pass
    else:
        raise ValueError('Extra reinterpretation instruction unexpectedly passed')
...
def main():
    ...
    parser.add_argument('--instruction-storage', action='store_true')
    ...
    headers = subprocess.run([args.objdump, '--section-headers', *args.objects], capture_output=True, text=True)
    if headers.returncode:
        raise ValueError(headers.stderr)
    prefix = '_' if 'file format mach-o' in headers.stdout else ''
    output = []
    names = NAMES + ([f'integer_codegen_{target}_{operation}'
        for target in ('fdq', 'fdq_vl') for operation in ('pack_words', 'identity', 'reinterpret')]
        if args.instruction_storage else [])
    bodies = {}
    for name in names:
    ...
    if args.instruction_storage:
        for target in ('fdq', 'fdq_vl'):
            check_identity_pair(bodies[f'integer_codegen_{target}_identity'],
                                bodies[f'integer_codegen_{target}_reinterpret'])
        print('Storage bit views match both raw identity boundaries exactly.')
    print(f'{len(names)} isolated integer/packing entries: native widths, no helper transfers; '
          'F/DQ entries contain only reviewed instructions, with or without VL')
