...
namespace integer_test::INTEGER_CASE_NAME {
  ...
  template<class T, std::size_t N> bool integer_shape() {
    ...
    static_assert(noexcept(simd::popcount(std::declval<V>())));
    ...
    for(unsigned round=0;round<259;++round) {
      ...
      simd::popcount(value).storeu(actual.data());
      for(std::size_t lane=0;lane<N;++lane)
        if(actual[lane]!=T(std::popcount(input[lane]))) return false;
      auto signed_value=simd::reinterpret_bits<std::make_signed_t<T>>(value);
      simd::reinterpret_bits<T>(signed_value).storeu(actual.data());
      ...
      if constexpr(sizeof(T)==4 && (N==2 || N==3)) {
        ...
        auto population=simd::popcount(poisoned);
        ...
        if(simd::reduce_add_widened(poisoned)!=sum) return false;
        if constexpr(N==2)
          if(simd::pairwise_add_widened(poisoned).to_native()!=sum) return false;
      }
      if constexpr(N>1 && N%2==0 && sizeof(T)<=4) {
        auto pairs=simd::pairwise_add_widened(value);
      ...
      if constexpr(sizeof(T)<=4) {
        ...
        for(std::size_t count=0;count<=N;++count) {
          ...
          if(simd::reduce_add_widened(prefix)!=expected) return false;
        }
      }
    }
    return true;
  ...
  template<class To,class From,std::size_t N> bool packing_shape() {
    using V=simd::vec<From,N,arch>;
    using R=simd::vec<To,2*N,arch>;
    static_assert(std::same_as<decltype(simd::narrow_concat<To>(std::declval<V>(),std::declval<V>())),R>);
    static_assert(noexcept(simd::narrow_concat<To>(std::declval<V>(),std::declval<V>())));
    ...
    for(unsigned round=0;round<257;++round) {
      ...
      simd::narrow_concat<To>(V::loadu(a.data()),V::loadu(b.data())).storeu(actual.data());
      for(std::size_t lane=0;lane<N;++lane)
