...
__attribute__((target("simd128"), noinline)) bool families() {
  return integers<std::int8_t>() && integers<std::uint8_t>() && integers<std::int16_t>() &&
         integers<std::uint16_t>() && integers<std::int32_t>() && integers<std::uint32_t>() &&
         integers<std::int64_t>() && integers<std::uint64_t>() && mixed_families() &&
         conversions() && floating_families<float>() && floating_families<double>();
}
int main() {
  if (!runtime())
    return 1;
