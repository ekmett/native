...
namespace bmi2_fixture {
  ...
  template<native::isa A, class U> concept rejects_unsigned_ops =
    !requires(U x) { native::bzhi<A>(x, 0u); } &&
    !requires(U x, U* high) { native::mulx<A>(x, x, high); } &&
    !requires(U x) { native::pdep<A>(x, x); } &&
    !requires(U x) { native::pext<A>(x, x); } &&
    !requires(U x) { native::shlx<A>(x, 0u); } &&
    !requires(U x) { native::shrx<A>(x, 0u); };
  ...
  static_assert(rejects_unsigned_ops<native::scalar, std::uint32_t> &&
                rejects_unsigned_ops<native::scalar, std::uint64_t>);
  static_assert(rejects_unsigned_ops<unrelated, std::uint32_t> &&
                rejects_unsigned_ops<unrelated, std::uint64_t>);
