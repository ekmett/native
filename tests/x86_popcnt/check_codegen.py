...
for index in range(1, len(objects), 2):
    ...
    if file_format.startswith("mach-o"):
        ...
        for address, section, name in symbols:
            if re.fullmatch(r"_?native_popcnt_u(?:16|32|64)", name):
...
for width in (16, 32, 64):
    name = f"native_popcnt_u{width}"
    body = bodies.get(name, "")
    if not re.search(r"\bpopcnt(?:w|l|q)?\b", body):
        raise SystemExit(f"{name} has no POPCNT instruction")
    if re.search(r"\bcallq?\s", body):
        raise SystemExit(f"{name} contains an unexpected helper call")
    if re.search(r"\b(?:[xyz]mm\d+|lzcnt[wlq]?|pdep[wlq]?|pext[wlq]?|bzhi[wlq]?|bextr[wlq]?)\b", body):
...
print("Unsigned 16/32/64-bit POPCNT fixtures contain native counts without helper calls.")
