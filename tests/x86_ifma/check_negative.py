...
"""Require a successful positive build and every IFMA boundary diagnostic."""
import argparse
...
parser.add_argument("--kind", choices=("feature", "target", "weak", "shape", "storage", "mask"), required=True)
...
if args.kind == "storage":
    ...
    raise SystemExit(0)
count = 0
operations = ["madd52lo", "mask_madd52lo", "maskz_madd52lo", "madd52hi", "mask_madd52hi", "maskz_madd52hi"]
if args.form.startswith("vex"):
    operations = ["madd52lo", "madd52hi"]
if args.kind == "mask":
    operations = [name for name in operations if name.startswith("mask")]
