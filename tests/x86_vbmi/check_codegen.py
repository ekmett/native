...
"""Check AVX512VBMI opcodes, widths, native masks and baseline importers."""
import argparse
...
parser.add_argument("--broad-target", action="store_true",
                    help="caller explicitly enables AVX512DQ as well as VBMI")
...
baseline = "native_vbmi_baseline_import" if args.baseline_only else "native_vbmi_baseline"
...
# Two-table plain/zero forms may use either tied-destination encoding.
# Merging forms must retain the designated first table or index destination.
allowed = re.compile(
    r"(?:kmov[bwdq]|vpermb|vperm[ti]2b|vpmultishiftqb|vzeroupper|"
...
for width, register in ((128, "xmm"), (256, "ymm"), (512, "zmm")):
    for opcode in ("vpermb", "vpermt2b", "vpermi2b", "vpmultishiftqb"):
        for variant in ("plain", "merge", "zero"):
            ...
            operations = [(mnemonic, operands) for mnemonic, operands in decoded
                          if mnemonic.startswith(("vperm", "vpmultishift"))]
            expected = {opcode}
            if opcode in ("vpermt2b", "vpermi2b") and variant != "merge":
                expected = {"vpermt2b", "vpermi2b"}
            if len(operations) != 1 or operations[0][0] not in expected:
                raise SystemExit(f"{name} must contain exactly one expected VBMI opcode.")
...
print("All 36 forms use the expected VBMI opcode, width and mask mode without helper calls.")
