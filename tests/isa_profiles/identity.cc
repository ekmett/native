...
template<simd::isa Arch,class... X>
concept isa_deducible=requires(X... x) { simd::vec(Arch,x...); };
static_assert(!isa_deducible<simd::avx2,float,float,float,float>);
static_assert(!isa_deducible<simd::avx2,std::array<float,4>>);

template<simd::isa Arch> struct family {
...
template<template<class,std::size_t,simd::isa> class Vector,simd::isa Arch>
...
static_assert(std::same_as<decltype(simd::vec<float,4,simd::avx2>(std::array<float,4>{})),simd::vec<float,4,simd::avx2>>);
