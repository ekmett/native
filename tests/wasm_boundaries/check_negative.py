...
"""Build matched Wasm SIMD128 positive and negative controls and retain diagnostics."""
import argparse
...
for target, must_succeed in ((args.positive, True), (args.negative, False)):
    ...
    if must_succeed and result.returncode:
        raise SystemExit("Wasm SIMD128 positive control failed:\n" + diagnostic)
    if not must_succeed and (result.returncode == 0 or not re.search(args.diagnostic, diagnostic, re.I)):
        raise SystemExit("Expected Wasm SIMD128 rejection diagnostic was absent:\n" + diagnostic)
print("Wasm SIMD128 positive and rejection controls passed: " + args.negative)
