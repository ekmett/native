...
#include <cstdint>
#include <cstdio>
constexpr auto partial_arch = native::feature_closure(native::avx2 & native::x86_feature::avx512f & native::x86_feature::avx512dq);
...
template<std::size_t N> bool check() {
  ...
  for(std::uint32_t bits=0;bits<(1u<<N);++bits) {
    ...
    auto packed=native::compress(mask,value,77u);packed.value.store(out.data());
    if(packed.count!=count || out!=expected) return false;
    native::expand(mask,packed.value,V(99u)).store(out.data());
    for(std::size_t lane=0;lane<N;++lane) if(out[lane]!=((bits&(1u<<lane))?input[lane]:99u)) return false;
    out.fill(55);auto written=native::compress_store(out.data(),3,mask,value);
