...
template<native::isa A, std::size_t N> using byte_vector = native::simd<std::uint8_t,N,A>;
template<native::isa A, std::size_t N> using matrix_vector = native::simd<std::uint64_t,N/8,A>;
template<native::isa A, std::size_t N, unsigned I = 0>
...
static_assert(!has_operations<native::scalar,16>);
