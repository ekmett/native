...
"""Build matched relaxed SIMD positive and negative controls and retain diagnostics."""
import argparse
...
for target, must_succeed in ((args.positive, True), (args.negative, False)):
    ...
    if must_succeed and result.returncode:
        raise SystemExit("relaxed SIMD positive control failed:\n" + diagnostic)
    if not must_succeed and (result.returncode == 0 or not re.search(args.diagnostic, diagnostic, re.I)):
        raise SystemExit("Expected relaxed SIMD rejection diagnostic was absent:\n" + diagnostic)
print("relaxed SIMD positive and rejection controls passed: " + args.negative)
