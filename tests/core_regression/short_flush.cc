...
template<std::size_t N> void check_short() {
  ...
  static_assert(std::same_as<decltype(native::flush_to_zero(F{})),F>);
  static_assert(noexcept(native::flush_to_zero(F{})));
  ...
  auto empty=native::flush_to_zero(std::array<F,0>{});
...
int main() {
  ...
  for(auto mode:{native::test::fp_mode::gradual,native::test::fp_mode::flush}) {
    ...
    require(native::test::read_fp_state()==before,"integer short flush modified FP state");
  }
  require(native::test::read_fp_state()==saved,"short flush fixture failed to restore FP state");
}
