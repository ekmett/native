...
void memory() {
  ...
  auto full = simd::load_simd<V>(input);
  auto tail = simd::load_simd_partial<V>(input.data(), 3, -1.f);
  std::array<float, 4> output{9.f, 9.f, 9.f, 9.f};
  simd::store_simd_partial(output.data(), tail, 3);
  check(output == std::array<float, 4>{1.f, 2.f, 3.f, 9.f});
  simd::store_simd(output.data(), full);
  check(output == input);
}
//! [memory]
...
void swizzles() {
  ...
  position.x = 4.f;
  check(all(saved == simd::vec<float, 2, Arch>{1.f, 2.f}));
...
void arithmetic() {
  ...
  check(all(simd::abs(V(-2.f)) == V(2.f)));
  check(all(simd::scaleb(V(1.f), V(3.5f)) == V(8.f)));
  auto integers = simd::convert<std::int32_t>(V(3.75f));
  check(all(simd::convert<float>(integers) == V(3.f)));
}
//! [arithmetic]
...
void rounding() {
  ...
  check(all(simd::floor(x) == V{-2.f, -1.f, 0.f, 1.f}));
  check(all(simd::ceil(x) == V{-1.f, -0.f, 1.f, 2.f}));
  check(all(simd::trunc(x) == V{-1.f, -0.f, 0.f, 1.f}));
}
//! [rounding]
...
void exponential() {
  ...
  auto result = simd::exp(registers);
...
void bit_transport() {
  ...
  auto zero = simd::flush_to_zero(tiny);
...
void wide_values() {
  ...
  simd::wide<float, 3> scalar_values{1.f, -2.f, 3.f};
  auto finite = simd::isfinite(scalar_values);
  static_assert(std::same_as<decltype(finite), simd::wide<bool, 3>>);
  check(finite.template get<0>() && finite.template get<1>());
  simd::wide<float, 0> empty{};
  check(simd::floor(empty).registers.empty());
}
//! [wide_values]
...
void common_utilities() {
  ...
  auto owned = simd::dup(first.data());
  if (owned) check(std::string_view(owned.get()) == "vector");
  check(!simd::type_of(first).empty());
  auto half = simd::fp16(1.5f);
...
//! [capabilities]
void capabilities() {
  auto maximum_leaf = simd::cpuid(0, 0).eax;
  ...
  int ready = 1;
  simd::wait_until<simd::spin>(&ready, [](volatile int * p) { return *p != 0; });
}
//! [capabilities]
#endif
}
