...
extern "C" __declspec(dllexport) __declspec(noinline) void add8(float const*a,float const*b,float*out){
  using V=native::simd<float,8,native::avx2>;auto result=V::load(a)+V::load(b);result.store(out);
}
#endif
#if defined(USE_AVX512)
...
extern "C" __declspec(dllexport) __declspec(noinline) void add16(float const*a,float const*b,float*out){
  using V=native::simd<float,16,native::avx512>;auto result=V::load(a)+V::load(b);result.store(out);
}
#endif
#if defined(USE_AVX2)&&defined(USE_AVX512)
static_assert(!std::same_as<native::simd<float,8,native::avx2>,native::simd<float,8,native::avx512>>);
static_assert(!std::same_as<fixture::engine<native::avx2,8>,fixture::engine<native::avx512,16>>);
static_assert(std::same_as<fixture::engine<native::avx2,8>::value_type,native::simd<float,8,native::avx2>>);
static_assert(std::same_as<fixture::engine<native::avx512,16>::value_type,native::simd<float,16,native::avx512>>);
#endif
int main(){
 ...
 using A=native::simd<float,8,native::avx2>;fixture::wide<A,3>aw{{A::load(a),A::load(a+8),A::load(a+16)}};
 ...
 using B=native::simd<float,16,native::avx512>;fixture::wide<B,3>bw{{B::load(b),B::load(b+16),B::load(b+32)}};
