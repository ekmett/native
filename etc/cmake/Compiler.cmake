...
block()
  ...
  ]=] NATIVE_HAS_CXX26_PACK_BINDINGS)
  if(NOT NATIVE_HAS_CXX26_PACK_BINDINGS)
    message(FATAL_ERROR "native requires C++26 structured-binding packs (qualified: upstream Clang 23).")
  endif()
endblock()
block()
  set(CMAKE_CXX_STANDARD 26)
  ...
  ]=] NATIVE_HAS_SWIZZLE_PROPERTIES)
  if(NOT NATIVE_HAS_SWIZZLE_PROPERTIES)
    message(FATAL_ERROR "native requires Clang named properties and explicit object parameters (-fms-extensions).")
  endif()
endblock()
...
block()
  ...
  foreach(format IN ITEMS FP16 BF16)
    ...
    " NATIVE_HAS_NATIVE_${format})
  endforeach()
endblock()
if(NATIVE_BUILD_TESTS)
  if(NOT NATIVE_TEST_ISA MATCHES "^(AVX2|AVX512|NEON|WASM_SIMD128|WASM_RELAXED_SIMD)$")
    message(FATAL_ERROR "NATIVE_TEST_ISA must be AVX2, AVX512, NEON, WASM_SIMD128, or WASM_RELAXED_SIMD.")
  endif()
  if(NATIVE_TEST_ISA MATCHES "^WASM")
    if(NOT CMAKE_SYSTEM_PROCESSOR MATCHES "^wasm(32|64)$")
      message(FATAL_ERROR "WASM profiles require a WebAssembly target toolchain.")
    endif()
  elseif(NATIVE_TEST_ISA STREQUAL "NEON")
    if(NOT CMAKE_SYSTEM_PROCESSOR MATCHES "^(aarch64|arm64|ARM64)$")
      message(FATAL_ERROR "NATIVE_TEST_ISA=NEON requires an arm64 target toolchain.")
    endif()
  elseif(NOT CMAKE_SYSTEM_PROCESSOR MATCHES "^(AMD64|amd64|x86_64|X86_64)$")
    message(FATAL_ERROR "NATIVE_TEST_ISA=${NATIVE_TEST_ISA} requires an x86-64 target toolchain.")
  endif()
endif()
if(NATIVE_BUILD_HOST AND NOT CMAKE_GENERATOR MATCHES "^Ninja")
  message(FATAL_ERROR "The qualified native module build requires Ninja or Ninja Multi-Config.")
endif()
if(NATIVE_ENABLE_IPO)
  include(CheckIPOSupported)
  check_ipo_supported(RESULT native_ipo_ok OUTPUT native_ipo_error LANGUAGES CXX)
  if(NOT native_ipo_ok)
    message(FATAL_ERROR "Requested IPO is unavailable: ${native_ipo_error}")
  endif()
endif()
...
if(CMAKE_SYSTEM_PROCESSOR MATCHES "^(AMD64|amd64|x86_64|X86_64)$")
  block()
    native_profile_options(AVX512_BF16 bf16_options)
    ...
    ]=] NATIVE_HAS_AVX512_BF16_INTRINSICS)
    if(NOT NATIVE_HAS_AVX512_BF16_INTRINSICS)
...
if(CMAKE_SYSTEM_PROCESSOR MATCHES "^(aarch64|arm64|ARM64)$")
  block()
    native_profile_options(NEON_BF16 bf16_options)
    string(JOIN " " bf16_flags ${bf16_options})
    string(APPEND CMAKE_REQUIRED_FLAGS " ${bf16_flags}")
    set(CMAKE_TRY_COMPILE_TARGET_TYPE STATIC_LIBRARY)
    check_cxx_source_compiles([=[
    ...
    ]=] NATIVE_HAS_NEON_BF16_INTRINSICS)
    if(NOT NATIVE_HAS_NEON_BF16_INTRINSICS)
...
if(CMAKE_SYSTEM_PROCESSOR MATCHES "^(AMD64|amd64|x86_64|X86_64)$")
  block()
    native_profile_options(AVX512_FP16 fp16_options)
    ...
    ]=] NATIVE_HAS_AVX512_FP16_INTRINSICS)
    if(NOT NATIVE_HAS_AVX512_FP16_INTRINSICS)
...
if(CMAKE_SYSTEM_PROCESSOR MATCHES "^(aarch64|arm64|ARM64)$")
  block()
    native_profile_options(NEON_FP16 fp16_options)
    string(JOIN " " fp16_flags ${fp16_options})
    string(APPEND CMAKE_REQUIRED_FLAGS " ${fp16_flags}")
    set(CMAKE_TRY_COMPILE_TARGET_TYPE STATIC_LIBRARY)
    check_cxx_source_compiles([=[
    ...
    ]=] NATIVE_HAS_NEON_FP16_INTRINSICS)
    if(NOT NATIVE_HAS_NEON_FP16_INTRINSICS)
      message(FATAL_ERROR "NEON_FP16 requires compiler support for native FP16 vector arithmetic.")
    endif()
  endblock()
endif()

function(native_host_settings target)
  ...
  if(CMAKE_CXX_COMPILER_FRONTEND_VARIANT STREQUAL "MSVC")
    ...
    if(NATIVE_ENABLE_ASAN)
      target_compile_options(${target} PUBLIC /fsanitize=address)
      target_link_options(${target} PUBLIC /INCREMENTAL:NO)
    endif()
  else()
    target_compile_options(${target} PUBLIC -fno-fast-math -ffp-contract=off)
    if(NOT CMAKE_SYSTEM_PROCESSOR MATCHES "^wasm(32|64)$")
  ...
  if(NOT NATIVE_ENABLE_EXCEPTIONS)
    if(CMAKE_CXX_COMPILER_FRONTEND_VARIANT STREQUAL "MSVC")
...
function(native_link_clang_cl_asan target)
  if(NOT NATIVE_ENABLE_ASAN)
    return()
  endif()
