...
function(native_profile_options profile output)
  set(flags)
  if(profile MATCHES "^WASM")
    list(APPEND flags -msimd128)
    if(profile STREQUAL "WASM_RELAXED_SIMD")
      list(APPEND flags -mrelaxed-simd)
    endif()
  elseif(NOT profile MATCHES "^NEON")
    list(APPEND flags -mavx2 -mfma)
  endif()
...
# Linking native::native alone does not select an ISA or a runtime dispatch policy.
function(native_target_profile target profile)
  if(NOT TARGET "${target}")
    message(FATAL_ERROR "native_target_profile requires an existing target: ${target}")
  endif()
  string(TOUPPER "${profile}" profile)
  if(NOT profile MATCHES "^(AVX2|AVX512|AVX512_BF16|AVX512_FP16|NEON|NEON_FP16|NEON_BF16|WASM_SIMD128|WASM_RELAXED_SIMD)$")
    message(FATAL_ERROR "Unknown native target profile: ${profile}")
  endif()
  if(profile MATCHES "^WASM")
    if(NOT CMAKE_SYSTEM_PROCESSOR MATCHES "^wasm(32|64)$")
      message(FATAL_ERROR "The WASM profile requires a WebAssembly target toolchain.")
    endif()
  ...
  get_target_property(previous "${target}" NATIVE_TARGET_PROFILE)
  ...
  set_property(TARGET "${target}" PROPERTY NATIVE_TARGET_PROFILE "${profile}")
  if(CMAKE_CXX_COMPILER_FRONTEND_VARIANT STREQUAL "MSVC" AND NOT CMAKE_CXX_COMPILER_ID STREQUAL "Clang")
    message(FATAL_ERROR "The Windows native profiles require clang-cl.")
  endif()
  native_profile_options("${profile}" flags)
  ...
  set_property(TARGET "${target}" PROPERTY NATIVE_SOURCE_ISA_OPTIONS "${flags}")
  cmake_language(EVAL CODE "cmake_language(DEFER CALL native_source_profile [[${target}]])")
endfunction()
...
function(native_target_omnibus target)
  if(NOT TARGET "${target}" OR NOT TARGET native::native)
    message(FATAL_ERROR "native_target_omnibus requires its target and native::native.")
  endif()
endfunction()

function(native_source_profile target)
  get_target_property(module_sets "${target}" CXX_MODULE_SETS)
  get_target_property(options "${target}" NATIVE_SOURCE_ISA_OPTIONS)
  ...
  foreach(source IN LISTS sources)
    ...
    if(NOT source MATCHES "^\\$<" AND source MATCHES "[.](cc|cpp|cxx|C)$")
      ...
      native_context_source_profile("${target}" "${source}")
    endif()
  endforeach()
  ...
  if(pch)
    ...
    native_context_source_profile("${target}" "${intermediate}/${pch_name}")
  endif()
endfunction()
...
function(native_context_source_profile target source)
  get_property(configured SOURCE "${source}" TARGET_DIRECTORY "${target}"
    PROPERTY NATIVE_CONTEXT_PROFILE_OPTIONS_SET)
  if(NOT configured)
    ...
    set_property(SOURCE "${source}" TARGET_DIRECTORY "${target}" PROPERTY
      COMPILE_OPTIONS "$<TARGET_PROPERTY:NATIVE_SOURCE_ISA_OPTIONS>" ${existing})
    set_property(SOURCE "${source}" TARGET_DIRECTORY "${target}" PROPERTY
      NATIVE_CONTEXT_PROFILE_OPTIONS_SET TRUE)
  endif()
endfunction()
