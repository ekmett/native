      ...
      noexcept([] {
        if constexpr (NATIVE_WIDE_DETAIL::wide_batch_assignable<wide<T,N>,B,NATIVE_WIDE_DETAIL::wide_add>)
          return noexcept(std::declval<wide<T,N> &>() = NATIVE_WIDE_DETAIL::wide_dispatch(
            std::declval<wide<T,N> const &>(),std::declval<B const &>(),NATIVE_WIDE_DETAIL::wide_add{}));
        else return false;
      }()) {
    if constexpr (NATIVE_WIDE_DETAIL::wide_batch_assignable<wide<T,N>,B,NATIVE_WIDE_DETAIL::wide_add>)
      return a = NATIVE_WIDE_DETAIL::wide_dispatch(a,b,NATIVE_WIDE_DETAIL::wide_add{});
    else return NATIVE_WIDE_DETAIL::wide_mutate(a, b, [](auto & x, auto const & y) { x += y; }, std::make_index_sequence<N>{});
  }
  ...
  native_inline constexpr wide<T, N> & operator-=(wide<T, N> & a, B const & b)
      noexcept([] {
        if constexpr (NATIVE_WIDE_DETAIL::wide_batch_assignable<wide<T,N>,B,NATIVE_WIDE_DETAIL::wide_sub>)
          return noexcept(std::declval<wide<T,N> &>() = NATIVE_WIDE_DETAIL::wide_dispatch(
            std::declval<wide<T,N> const &>(),std::declval<B const &>(),NATIVE_WIDE_DETAIL::wide_sub{}));
        else return false;
      }()) {
    if constexpr (NATIVE_WIDE_DETAIL::wide_batch_assignable<wide<T,N>,B,NATIVE_WIDE_DETAIL::wide_sub>)
      return a = NATIVE_WIDE_DETAIL::wide_dispatch(a,b,NATIVE_WIDE_DETAIL::wide_sub{});
    else return NATIVE_WIDE_DETAIL::wide_mutate(a, b, [](auto & x, auto const & y) { x -= y; }, std::make_index_sequence<N>{});
  }
  ...
  native_inline constexpr wide<T, N> & operator*=(wide<T, N> & a, B const & b)
      noexcept([] {
        if constexpr (NATIVE_WIDE_DETAIL::wide_batch_assignable<wide<T,N>,B,NATIVE_WIDE_DETAIL::wide_mul>)
          return noexcept(std::declval<wide<T,N> &>() = NATIVE_WIDE_DETAIL::wide_dispatch(
            std::declval<wide<T,N> const &>(),std::declval<B const &>(),NATIVE_WIDE_DETAIL::wide_mul{}));
        else return false;
      }()) {
    if constexpr (NATIVE_WIDE_DETAIL::wide_batch_assignable<wide<T,N>,B,NATIVE_WIDE_DETAIL::wide_mul>)
      return a = NATIVE_WIDE_DETAIL::wide_dispatch(a,b,NATIVE_WIDE_DETAIL::wide_mul{});
    else return NATIVE_WIDE_DETAIL::wide_mutate(a, b, [](auto & x, auto const & y) { x *= y; }, std::make_index_sequence<N>{});
  }
  ...
  native_nodiscard native_inline constexpr wide<R,N> abs(wide<R,N> const & input)
      noexcept([] {
        if constexpr (requires(std::array<R,N> const & a) { NATIVE_WIDE_DETAIL::adl_abs(a); })
          ...
          (N == 0 || noexcept(std::declval<R &>() = NATIVE_WIDE_DETAIL::adl_abs(std::declval<R const &>())));
      }()) {
    if constexpr (requires { NATIVE_WIDE_DETAIL::adl_abs(input.registers); })
      return wide<R,N>{NATIVE_WIDE_DETAIL::adl_abs(input.registers)};
    else {
      wide<R,N> result;
      auto & [...value] = result;
      auto const & [...x] = input;
      ([&] { value = NATIVE_WIDE_DETAIL::adl_abs(x); }(), ...);
      return result;
    }
  }
  /// \ingroup wide_values
  ...
  native_nodiscard native_inline constexpr wide<R,N> sqrt(wide<R,N> const & input)
      noexcept([] {
        if constexpr (requires(std::array<R,N> const & a) { NATIVE_WIDE_DETAIL::adl_sqrt(a); })
          ...
          (N == 0 || noexcept(std::declval<R &>() = NATIVE_WIDE_DETAIL::adl_sqrt(std::declval<R const &>())));
      }()) {
    if constexpr (requires { NATIVE_WIDE_DETAIL::adl_sqrt(input.registers); })
      return wide<R,N>{NATIVE_WIDE_DETAIL::adl_sqrt(input.registers)};
    else {
      wide<R,N> result;
      auto & [...value] = result;
      auto const & [...x] = input;
      ([&] { value = NATIVE_WIDE_DETAIL::adl_sqrt(x); }(), ...);
      return result;
    }
  }
  /// \ingroup wide_values
  ...
  native_nodiscard native_inline constexpr wide<R,N> sin(wide<R,N> const & input)
      noexcept([] {
        if constexpr (requires(std::array<R,N> const & a) { NATIVE_WIDE_DETAIL::adl_sin(a); })
          ...
          (N == 0 || noexcept(std::declval<R &>() = NATIVE_WIDE_DETAIL::adl_sin(std::declval<R const &>())));
      }()) {
    if constexpr (requires { NATIVE_WIDE_DETAIL::adl_sin(input.registers); })
      return wide<R,N>{NATIVE_WIDE_DETAIL::adl_sin(input.registers)};
    else {
      wide<R,N> result;
      auto & [...value] = result;
      auto const & [...x] = input;
      ([&] { value = NATIVE_WIDE_DETAIL::adl_sin(x); }(), ...);
      return result;
    }
  }
  /// \ingroup wide_values
  ...
  native_nodiscard native_inline constexpr wide<R,N> cos(wide<R,N> const & input)
      noexcept([] {
        if constexpr (requires(std::array<R,N> const & a) { NATIVE_WIDE_DETAIL::adl_cos(a); })
          ...
          (N == 0 || noexcept(std::declval<R &>() = NATIVE_WIDE_DETAIL::adl_cos(std::declval<R const &>())));
      }()) {
    if constexpr (requires { NATIVE_WIDE_DETAIL::adl_cos(input.registers); })
      return wide<R,N>{NATIVE_WIDE_DETAIL::adl_cos(input.registers)};
    else {
      wide<R,N> result;
      auto & [...value] = result;
      auto const & [...x] = input;
      ([&] { value = NATIVE_WIDE_DETAIL::adl_cos(x); }(), ...);
      return result;
    }
  }
  /// \ingroup wide_values
  ...
  native_nodiscard native_inline constexpr wide<R,N> expm1(wide<R,N> const & input)
      noexcept([] {
        if constexpr (requires(std::array<R,N> const & a) { NATIVE_WIDE_DETAIL::adl_expm1(a); })
          ...
          (N == 0 || noexcept(std::declval<R &>() = NATIVE_WIDE_DETAIL::adl_expm1(std::declval<R const &>())));
      }()) {
    if constexpr (requires { NATIVE_WIDE_DETAIL::adl_expm1(input.registers); })
      return wide<R,N>{NATIVE_WIDE_DETAIL::adl_expm1(input.registers)};
    else {
      wide<R,N> result;
      auto & [...value] = result;
      auto const & [...x] = input;
  ...
  native_nodiscard native_inline constexpr wide<R,N> log(wide<R,N> const & input)
      noexcept([] {
        if constexpr (requires(std::array<R,N> const & a) { NATIVE_WIDE_DETAIL::adl_log(a); })
          ...
          (N == 0 || noexcept(std::declval<R &>() = NATIVE_WIDE_DETAIL::adl_log(std::declval<R const &>())));
      }()) {
    if constexpr (requires { NATIVE_WIDE_DETAIL::adl_log(input.registers); })
      return wide<R,N>{NATIVE_WIDE_DETAIL::adl_log(input.registers)};
    else {
      wide<R,N> result;
      auto & [...value] = result;
      auto const & [...x] = input;
  ...
  native_nodiscard native_inline constexpr wide<R,N> log1p(wide<R,N> const & input)
      noexcept([] {
        if constexpr (requires(std::array<R,N> const & a) { NATIVE_WIDE_DETAIL::adl_log1p(a); })
          ...
          (N == 0 || noexcept(std::declval<R &>() = NATIVE_WIDE_DETAIL::adl_log1p(std::declval<R const &>())));
      }()) {
    if constexpr (requires { NATIVE_WIDE_DETAIL::adl_log1p(input.registers); })
      return wide<R,N>{NATIVE_WIDE_DETAIL::adl_log1p(input.registers)};
    else {
      wide<R,N> result;
      auto & [...value] = result;
      auto const & [...x] = input;
      ([&] { value = NATIVE_WIDE_DETAIL::adl_log1p(x); }(), ...);
      return result;
    }
  }
  /// \ingroup wide_values
  ...
  native_nodiscard native_inline constexpr wide<R,N> tanh(wide<R,N> const & input)
      noexcept([] {
        if constexpr (requires(std::array<R,N> const & a) { NATIVE_WIDE_DETAIL::adl_tanh(a); })
          ...
          (N == 0 || noexcept(std::declval<R &>() = NATIVE_WIDE_DETAIL::adl_tanh(std::declval<R const &>())));
      }()) {
    if constexpr (requires { NATIVE_WIDE_DETAIL::adl_tanh(input.registers); })
      return wide<R,N>{NATIVE_WIDE_DETAIL::adl_tanh(input.registers)};
    else {
      wide<R,N> result;
      auto & [...value] = result;
      auto const & [...x] = input;
      ([&] { value = NATIVE_WIDE_DETAIL::adl_tanh(x); }(), ...);
      return result;
    }
  }
  /// \ingroup wide_values
  ...
  native_nodiscard native_inline constexpr wide<R,N> atan2(wide<R,N> const & y,wide<R,N> const & x)
      noexcept([] {
        if constexpr (requires(std::array<R,N> const & a) { NATIVE_WIDE_DETAIL::adl_atan2(a,a); })
          ...
          (N == 0 || noexcept(std::declval<R &>() = NATIVE_WIDE_DETAIL::adl_atan2(
            std::declval<R const &>(),std::declval<R const &>())));
      }()) {
    if constexpr (requires { NATIVE_WIDE_DETAIL::adl_atan2(y.registers,x.registers); })
      return wide<R,N>{NATIVE_WIDE_DETAIL::adl_atan2(y.registers,x.registers)};
    else {
      wide<R,N> result;
      auto & [...value] = result;
      auto const & [...a] = y;
      auto const & [...b] = x;
      ([&] { value = NATIVE_WIDE_DETAIL::adl_atan2(a,b); }(), ...);
      return result;
    }
  }
  /// \ingroup wide_values
  ...
  native_nodiscard native_inline constexpr wide<R,N> fma(wide<R,N> const & a,wide<R,N> const & b,wide<R,N> const & c)
      noexcept([] {
        if constexpr (requires(std::array<R,N> const & x) { NATIVE_WIDE_DETAIL::adl_fma(x,x,x); })
          ...
          (N == 0 || noexcept(std::declval<R &>() = NATIVE_WIDE_DETAIL::adl_fma(
            std::declval<R const &>(),std::declval<R const &>(),std::declval<R const &>())));
      }()) {
    if constexpr (requires { NATIVE_WIDE_DETAIL::adl_fma(a.registers,b.registers,c.registers); })
      return wide<R,N>{NATIVE_WIDE_DETAIL::adl_fma(a.registers,b.registers,c.registers)};
    else {
      wide<R,N> result;
      auto & [...value] = result;
      auto const & [...x] = a;
      auto const & [...y] = b;
      auto const & [...z] = c;
      ([&] { value = NATIVE_WIDE_DETAIL::adl_fma(x,y,z); }(), ...);
      return result;
    }
  }
  /// \ingroup wide_values
  ...
  native_nodiscard native_inline constexpr auto sincos(wide<R,N> const & input)
      noexcept([] {
        if constexpr (requires(std::array<R,N> const & x) { NATIVE_WIDE_DETAIL::adl_sincos(x); })
          ...
          (N == 0 || (noexcept(NATIVE_WIDE_DETAIL::adl_sincos(std::declval<R const &>())) && noexcept(std::declval<R &>() = std::declval<decltype(NATIVE_WIDE_DETAIL::adl_sincos(std::declval<R const &>())) &>().first) &&
            noexcept(std::declval<R &>() = std::declval<decltype(NATIVE_WIDE_DETAIL::adl_sincos(std::declval<R const &>())) &>().second)));
      }()) {
    if constexpr (requires { NATIVE_WIDE_DETAIL::adl_sincos(input.registers); }) {
      auto [sine,cosine] = NATIVE_WIDE_DETAIL::adl_sincos(input.registers);
      ...
      auto & [...c] = cosine;
      auto const & [...x] = input;
      ([&] { auto pair = NATIVE_WIDE_DETAIL::adl_sincos(x); s = pair.first; c = pair.second; }(), ...);
  ...
  native_nodiscard native_inline constexpr wide<R, N> select(wide<decltype(R{} == R{}), N> const & masks,
      ...
      noexcept([] {
        if constexpr (N == 0)
          ...
          return noexcept(wide<R,N>()) && std::is_nothrow_move_constructible_v<wide<R,N>> &&
            noexcept(std::declval<R &>() = NATIVE_WIDE_DETAIL::adl_select(
    ...
    if constexpr (std::is_same_v<decltype(R{} == R{}), bool>)
      ...
      ([&] { value = NATIVE_WIDE_DETAIL::adl_select(mask, x, y); }(), ...);
    return result;
  }
