...
// Intentionally repeatable under one target family.
  public:
    ...
    NATIVE_WIDE_TARGET native_inline constexpr wide()
      noexcept(std::is_nothrow_default_constructible_v<std::array<T,N>>)
      requires (detail::wide_target<T> == NATIVE_WIDE_INDEX) && detail::wide_target_default<T,N>
      : registers{} {}
    /// Take an array of exactly the pack extent.
    NATIVE_WIDE_TARGET native_inline constexpr wide(std::array<T, N> values)
      noexcept(std::is_nothrow_move_constructible_v<std::array<T, N>>)
      requires (detail::wide_target<T> == NATIVE_WIDE_INDEX)
      : registers(std::move(values)) {}
    /// Construct one element from each argument, preserving argument order.
    template<class... U> requires (detail::wide_target<T,U...> == NATIVE_WIDE_INDEX) && (sizeof...(U) == N && N != 0) &&
      (std::constructible_from<T,U &&> && ...) &&
      (N != 1 || ((!detail::wide_traits<std::remove_cvref_t<U>>::value &&
        !std::same_as<std::remove_cvref_t<U>,std::array<T,N>>) && ...))
    explicit(N == 1) NATIVE_WIDE_TARGET native_inline constexpr wide(U &&... values)
      noexcept((std::is_nothrow_constructible_v<T,U &&> && ...))
      : registers{T(std::forward<U>(values))...} {}
    /// Convert one value to `T`, then copy it across the pack.
    template<class U> requires (detail::wide_target<T,U> == NATIVE_WIDE_INDEX) && (N != 1) && (!detail::wide_traits<std::remove_cvref_t<U>>::value) &&
      (!std::same_as<std::remove_cvref_t<U>,std::array<T, N>>) &&
      std::constructible_from<T,U const &> && (N == 0 || std::copy_constructible<T>)
    explicit NATIVE_WIDE_TARGET native_inline constexpr wide(U const & value)
    ...
    template<class U> requires (detail::wide_target<T,U> == NATIVE_WIDE_INDEX) && (!std::same_as<T,U>) && std::constructible_from<T,U const &>
    explicit(!std::is_convertible_v<U const &,T>) NATIVE_WIDE_TARGET native_inline constexpr wide(wide<U,N> const & source)
      noexcept(N == 0 || std::is_nothrow_constructible_v<T,U const &>)
      : wide(conversion_tag{}, source, std::make_index_sequence<N>{}) {}
    /// Convert each element of a same-size pack; explicitness follows the element conversion.
    template<class U> requires (detail::wide_target<T,U> == NATIVE_WIDE_INDEX) && (!std::same_as<T,U>) && std::constructible_from<T,U &&>
    explicit(!std::is_convertible_v<U &&,T>) NATIVE_WIDE_TARGET native_inline constexpr wide(wide<U,N> && source)
    ...
    template<class F> requires (detail::wide_target<T> == NATIVE_WIDE_INDEX)
    NATIVE_WIDE_TARGET native_inline constexpr decltype(auto) apply(F && f) noexcept(noexcept(std::apply(std::forward<F>(f), registers))) {
      return apply_indices(std::forward<F>(f), registers, std::make_index_sequence<N>{});
    }

    /// Invoke `f` with all elements as arguments; the empty pack calls `f()`.
    template<class F> requires (detail::wide_target<T> == NATIVE_WIDE_INDEX)
    NATIVE_WIDE_TARGET native_inline constexpr decltype(auto) apply(F && f) const noexcept(noexcept(std::apply(std::forward<F>(f), registers))) {
      return apply_indices(std::forward<F>(f), registers, std::make_index_sequence<N>{});
    }
  private:
    template<class W, std::size_t... I> requires (detail::wide_target<T,W> == NATIVE_WIDE_INDEX)
    NATIVE_WIDE_TARGET native_inline constexpr wide(conversion_tag, W && source, std::index_sequence<I...>)
      noexcept((std::is_nothrow_constructible_v<T,
        decltype(std::get<I>(std::forward<W>(source).registers))> && ...))
      : registers{T(std::get<I>(std::forward<W>(source).registers))...} {}
    template<class U, std::size_t... I> requires (detail::wide_target<T> == NATIVE_WIDE_INDEX) && std::same_as<std::remove_cvref_t<U>,T>
    NATIVE_WIDE_TARGET native_inline constexpr wide(U && value, std::index_sequence<I...>)
      noexcept(N == 0 || std::is_nothrow_copy_constructible_v<T>)
    ...
    template<class F, class A, std::size_t ... I> requires (detail::wide_target<A> == NATIVE_WIDE_INDEX)
    static NATIVE_WIDE_TARGET native_inline constexpr decltype(auto) apply_indices(F && f, A & a, std::index_sequence<I...>) {
