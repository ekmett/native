...
namespace native {
  ...
  template<::NATIVE_BACKEND_NAMESPACE::short_element T,std::size_t N,::native::isa Arch>
    requires NATIVE_ARCH_REQUIRES(Arch) &&(N==2 || N==3) && requires { typename vec<T,4,Arch>::native_type; }
  ...
  struct alignas(typename vec<T,4,Arch>::native_type) vec<T,N,Arch> : detail::swizzle_access<T,N,Arch> {
    ...
    native_inline constexpr vec & operator=(vec const &) = default;
    /// Broadcast the supplied value to each logical lane. Unused physical lanes are zero.
    native_inline constexpr vec(T x) noexcept : value{::NATIVE_BACKEND_NAMESPACE::short_word(x),::NATIVE_BACKEND_NAMESPACE::short_word(x),
      N==3?::NATIVE_BACKEND_NAMESPACE::short_word(x):NATIVE_BACKEND_NAMESPACE::short_lane<T>(0),0} {}
    /// Broadcast the supplied value to each logical lane. Unused physical lanes are zero.
    explicit native_inline constexpr vec(bool x) noexcept requires simd_mask_element<T> : vec(T(x)) {}
    /// Adopt native lane storage and clear physical padding. Mask elements must already be canonical.
    native_inline constexpr vec(native_type x) noexcept
    ...
    native_inline constexpr vec(X... x) noexcept((noexcept(static_cast<T>(x)) && ...))
      : value{::NATIVE_BACKEND_NAMESPACE::short_word(static_cast<T>(x))...} {}
    /// Copy one value per logical lane in array order.
    native_inline vec(std::array<T,N> const & values) noexcept : vec(load(values.data())) {}
    ...
    native_nodiscard static native_inline vec from_native(native_type x) noexcept {
    ...
    native_nodiscard static native_inline constexpr vec unsafe_from_native(native_type x) noexcept { return vec(x); }
    ...
    native_nodiscard static native_inline vec from_storage(storage_type x) noexcept {
    ...
    native_nodiscard static native_inline vec load(T const * p) noexcept { return load_memory(p); }
    /// Load exactly the logical lanes without an additional alignment requirement.
    native_nodiscard static native_inline vec loadu(T const * p) noexcept { return load_memory(p); }
    ...
    native_nodiscard static native_inline vec load_partial(T const * p,std::size_t n,T fill={}) noexcept {
    ...
    native_nodiscard static native_inline vec from_bits(bits_type x) noexcept requires std::same_as<T,float> {
      return from_storage(storage_type::from_bits(x.to_storage()));
    }
    /// Reinterpret binary32 words as lane values without normalization; a scalar word is broadcast.
    native_nodiscard static native_inline vec from_bits(std::uint32_t x) noexcept requires std::same_as<T,float> { return from_bits(bits_type(x)); }
    /// Broadcast the float value without adding an FTZ or other normalization policy.
    native_nodiscard static native_inline vec from_float(float x) noexcept requires std::same_as<T,float> { return vec(x); }
    /// Adopt raw float storage without numerical conversion or normalization.
    native_nodiscard static native_inline vec unsafe_from_float32(native_type x) noexcept requires std::same_as<T,float> { return vec(x); }
    /// Load exactly the logical count of binary32 words without normalizing their representations.
    native_nodiscard static native_inline vec load_bits(std::uint32_t const * p) noexcept requires std::same_as<T,float> { return from_bits(bits_type::load(p)); }
    ...
    native_nodiscard static native_inline vec load_bits_partial(std::uint32_t const * p,std::size_t n,std::uint32_t fill=0) noexcept requires std::same_as<T,float> { return from_bits(bits_type::load_partial(p,n,fill)); }
    ...
    native_nodiscard static native_inline vec from_bitset(std::uint64_t bits) noexcept requires simd_mask_element<T> { return from_storage(storage_type::from_bitset(bits&lane_mask)); }
    ...
    native_nodiscard friend native_inline bool any(vec x) noexcept requires simd_mask_element<T> { return x.to_bitset()!=0; }
    /// Return true exactly when every logical lane is true.
    native_nodiscard friend native_inline bool all(vec x) noexcept requires simd_mask_element<T> { return x.to_bitset()==lane_mask; }
    /// Return true exactly when no logical lane is true.
    native_nodiscard friend native_inline bool none(vec x) noexcept requires simd_mask_element<T> { return !any(x); }
    ...
    native_nodiscard friend native_inline vec operator+(vec a,vec b) noexcept requires(!simd_mask_element<T>) { return clean(a.to_storage()+b.to_storage()); }
    ...
    native_nodiscard friend native_inline vec operator/(vec a,vec b) noexcept requires std::same_as<T,float> {
    ...
    native_nodiscard friend native_inline vec operator-(vec a) noexcept requires(!simd_mask_element<T>) { return from_storage(-a.to_storage()); }
    /// Bitwise AND of corresponding lane representations.
    native_nodiscard friend native_inline vec operator&(vec a,vec b) noexcept requires(!std::same_as<T,float>) { return from_storage(a.to_storage()&b.to_storage()); }
    ...
    native_nodiscard friend native_inline vec operator~(vec a) noexcept requires(!std::same_as<T,float>) { return from_storage(~a.to_storage()); }
    /// Return the lane-wise logical complement, retaining this mask type.
    native_nodiscard friend native_inline vec operator!(vec a) noexcept requires simd_mask_element<T> { return ~a; }
    /// Return a mask whose lanes are true where `a == b` holds.
    native_nodiscard friend native_inline mask_type operator==(vec a,vec b) noexcept { return comparison(a.to_storage()==b.to_storage()); }
    /// Return a mask whose lanes are true where `a != b` holds.
    native_nodiscard friend native_inline mask_type operator!=(vec a,vec b) noexcept { return ~(a==b); }
    /// Return a mask whose lanes are true where `a < b` holds.
    native_nodiscard friend native_inline mask_type operator<(vec a,vec b) noexcept requires(!simd_mask_element<T>) { return comparison(a.to_storage()<b.to_storage()); }
    /// Return a mask whose lanes are true where `a > b` holds.
    native_nodiscard friend native_inline mask_type operator>(vec a,vec b) noexcept requires(!simd_mask_element<T>) { return comparison(a.to_storage()>b.to_storage()); }
    /// Return a mask whose lanes are true where `a <= b` holds.
    native_nodiscard friend native_inline mask_type operator<=(vec a,vec b) noexcept requires(!simd_mask_element<T>) { return (a<b)|(a==b); }
    /// Return a mask whose lanes are true where `a >= b` holds.
    native_nodiscard friend native_inline mask_type operator>=(vec a,vec b) noexcept requires(!simd_mask_element<T>) { return (a>b)|(a==b); }
    ...
    native_nodiscard friend native_inline vec select(M mask,vec a,vec b) noexcept {
    ...
    native_nodiscard friend native_inline vec fma(vec a,vec b,vec c) noexcept requires std::same_as<T,float> { return clean(fma(a.to_storage(),b.to_storage(),c.to_storage())); }
    /// Compute the native square root of each logical lane in the caller's floating-point environment.
    native_nodiscard friend native_inline vec sqrt(vec a) noexcept requires std::same_as<T,float> { return clean(sqrt(a.to_storage())); }
    /// Round each logical lane to an integral value, ties to even, independently of the ambient rounding direction.
    native_nodiscard friend native_inline vec round_even(vec a) noexcept requires std::same_as<T,float> { return clean(round_even(a.to_storage())); }
    /// Construct normal powers of two; require each input to be an integral exponent in [-126,127].
    native_nodiscard friend native_inline vec normal_pow2(vec a) noexcept requires std::same_as<T,float> { return from_storage(normal_pow2(a.to_storage())); }
    ...
    native_nodiscard friend native_inline vec operator<<(vec a,imm_t<K>) noexcept requires simd_integer_element<T> { return from_storage(a.to_storage()<<imm<K>); }
    ...
    native_nodiscard native_inline vec left() const noexcept { return *this<<imm<K>; }
    /// Shift each lane right by compile-time K; signed lanes extend their sign. Require K below the lane bit width.
    template<unsigned K> requires(K<32) && simd_integer_element<T>
    native_nodiscard native_inline vec right() const noexcept { return *this>>imm<K>; }
    ...
    native_inline vec & operator+=(vec b) noexcept requires(!simd_mask_element<T>) { return *this=*this+b; }
    ...
    native_inline vec & operator/=(vec b) noexcept requires std::same_as<T,float> { return *this=*this/b; }
  ...
  private:
    struct unchecked {};
    native_inline constexpr vec(unchecked,native_type x) noexcept : value(x) {}
    native_nodiscard static native_inline vec clean(storage_type x) noexcept { return vec(unchecked{},std::bit_cast<native_type>(x.to_native())); }
  ...
  native_nodiscard native_inline vec<T,N,Arch> bit_select(vec<T,N,Arch> bits,vec<T,N,Arch> a,vec<T,N,Arch> b) noexcept { return (bits&a)|(~bits&b); }
  ...
  native_nodiscard native_inline vec<T,N,Arch> masked_add(M m,vec<T,N,Arch> prior,vec<T,N,Arch> a,vec<T,N,Arch> b) noexcept { return select(m,a+b,prior); }
  /// Subtract matching integer lanes, retaining prior in inactive mask lanes; arithmetic wraps at the lane width.
  template<simd_integer_element T,std::size_t N,::native::isa Arch,class M>
    requires NATIVE_ARCH_REQUIRES(Arch) &&(N==2 || N==3) && (sizeof(T)==4) &&
      (std::same_as<M,typename vec<T,N,Arch>::mask> || std::same_as<M,vec<mask32,N,Arch>>)
  native_nodiscard native_inline vec<T,N,Arch> masked_sub(M m,vec<T,N,Arch> prior,vec<T,N,Arch> a,vec<T,N,Arch> b) noexcept { return select(m,a-b,prior); }
  /// Multiply matching integer lanes, retaining prior in inactive mask lanes; arithmetic wraps at the lane width.
  template<simd_integer_element T,std::size_t N,::native::isa Arch,class M>
    requires NATIVE_ARCH_REQUIRES(Arch) &&(N==2 || N==3) && (sizeof(T)==4) &&
      (std::same_as<M,typename vec<T,N,Arch>::mask> || std::same_as<M,vec<mask32,N,Arch>>)
  native_nodiscard native_inline vec<T,N,Arch> masked_mul(M m,vec<T,N,Arch> prior,vec<T,N,Arch> a,vec<T,N,Arch> b) noexcept { return select(m,a*b,prior); }
}
