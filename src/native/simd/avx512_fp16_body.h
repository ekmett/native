...
export namespace native {
  ...
  template<::native::isa Arch> requires NATIVE_ARCH_REQUIRES(Arch) struct vec<fp16,32,Arch> {
  ...
  public:
    ...
    native_inline explicit vec(fp16 value) noexcept
    ...
    native_inline explicit vec(std::array<fp16,lanes> const & values) noexcept : vec(load(values.data())) {}
    ...
    native_inline vec(T... values) noexcept : vec(std::array<fp16,lanes>{values...}) {}
    ...
    native_nodiscard static native_inline vec load_memory(fp16 const * p) noexcept {
    ...
    native_nodiscard static native_inline vec load(fp16 const * p) noexcept { return load_memory(p); }
    ...
    native_nodiscard static native_inline vec loadu(fp16 const * p) noexcept { return load(p); }
    ...
    native_nodiscard static native_inline vec load_partial(fp16 const * p, std::size_t n,
    ...
    native_nodiscard friend native_inline vec operator+(vec a,vec b) noexcept {
    ...
    /// The caller's MXCSR rounding control and native exception behavior apply.
    native_nodiscard friend native_inline vec sqrt(vec a) noexcept {
    ...
    native_nodiscard friend native_inline vec operator-(vec a) noexcept {
    ...
    native_nodiscard friend native_inline mask operator==(vec a,vec b) noexcept {
    ...
    native_nodiscard friend native_inline mask operator!=(vec a,vec b) noexcept { return ~(a==b); }
    ...
    native_nodiscard friend native_inline mask operator>(vec a,vec b) noexcept { return b<a; }
    ...
    native_nodiscard friend native_inline vec select(mask m,vec a,vec b) noexcept {
  ...
  native_nodiscard native_inline vec<fp16,32,Arch> fma(
