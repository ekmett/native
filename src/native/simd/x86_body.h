...
namespace native {
  ...
  template <::native::isa<> Arch> requires NATIVE_ARCH_REQUIRES(Arch) struct native_empty_bases simd<float, 4,Arch> : detail::register_memory<simd<float,4,Arch>, 4>, detail::swizzle_access<float,4,Arch> {
    ...
    native_artificial native_nodiscard static native_inline native_const simd from_bits(bits_type bits) noexcept { return simd(_mm_castsi128_ps(bits.value)); }
    /// Reinterpret unsigned lane words as binary32, without normalization.
  ...
  template <::native::isa<> Arch> requires NATIVE_ARCH_REQUIRES(Arch) struct simd<float, 8,Arch> : detail::register_memory<simd<float,8,Arch>, 8> {
    ...
    native_nodiscard native_inline native_pure bits_type to_bits() const noexcept { return bits(); }
    /// Reinterpret unsigned lane words as binary32, without normalization.
    native_artificial native_nodiscard static native_inline native_const simd from_bits(bits_type bits) noexcept { return simd(_mm256_castsi256_ps(bits.value)); }
    /// Reinterpret unsigned lane words as binary32, without normalization.
    native_nodiscard static native_inline native_const simd from_bits(std::uint32_t bits) noexcept { return from_bits(bits_type(bits)); }
    /// Broadcast one binary32 value to every lane.
    native_nodiscard static native_inline native_const simd from_float(float x) noexcept { return simd(x); }
    ...
    native_inline void store_bits_partial(native_noescape std::uint32_t * p,std::size_t n) const noexcept { bits().store_partial(p,n); }
    /// Load one lane from each array element, in array order.
    native_inline simd(std::array<float,8> const & values) noexcept : simd(loadu(values.data())) {}
#if defined(__clang__)
    /// Convert one argument per lane; exceptions follow those named-lvalue conversions.
    ...
    /// Convert one argument per lane; exceptions follow those named-lvalue conversions.
    template <class... X> requires (sizeof...(X)==8) && (std::convertible_to<X,float> && ...)
    native_inline simd(X... x) noexcept((noexcept(static_cast<float>(x)) && ...)) : simd(loadu(std::array<float,8>{static_cast<float>(x)...}.data())) {}
#endif
    /// Apply the corresponding lane-wise add operation in place and return *this.
    native_inline simd & operator+=(simd b) noexcept { return *this=*this+b; }
    /// Apply the corresponding lane-wise subtract operation in place and return *this.
