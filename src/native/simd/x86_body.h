...
namespace native {
  ...
  template <::native::isa Arch> requires NATIVE_ARCH_REQUIRES(Arch) struct native_empty_bases vec<float, 4,Arch> : detail::register_memory<vec<float,4,Arch>, 4>, detail::swizzle_access<float,4,Arch> {
    static constexpr isa architecture=Arch;
    template <class T> using rebind = vec<T,4,Arch>;
    using vector_mask_type=vec<mask32,4,Arch>;
    using mask_type=std::conditional_t<bool(NATIVE_HAS_AVX512VL),predicate<4,Arch>,vec<mask32,4,Arch>>;
    ...
    native_inline vec(float x) : value(_mm_set1_ps(x)) {}
    /// Adopt native lane storage without numerical conversion.
    native_inline constexpr vec(__m128 x) : value(x) {}
    /// Load every logical lane; no extra alignment is required.
    native_nodiscard static native_inline native_pure vec load(native_noescape float const * p) { return load_memory<1>(p); }
    /// Store every logical lane; no extra alignment is required.
    native_inline void store(native_noescape float * p) const { store_memory<1>(p); }
    /// Add corresponding floating-point lanes using the caller's rounding and denormal environment.
    native_artificial native_nodiscard friend native_inline native_pure vec operator+(vec a, vec b) { return vec(_mm_add_ps(a.value, b.value)); }
    /// Subtract corresponding floating-point lanes using the caller's rounding and denormal environment.
    native_artificial native_nodiscard friend native_inline native_pure vec operator-(vec a, vec b) { return vec(_mm_sub_ps(a.value, b.value)); }
    /// Multiply corresponding floating-point lanes using the caller's rounding and denormal environment.
    native_artificial native_nodiscard friend native_inline native_pure vec operator*(vec a, vec b) { return vec(_mm_mul_ps(a.value, b.value)); }
    /// Divide corresponding floating-point lanes using the caller's rounding and denormal environment.
    native_artificial native_nodiscard friend native_inline native_pure vec operator/(vec a, vec b) { return vec(_mm_div_ps(a.value, b.value)); }
    /// Negate every logical lane; floating-point lanes change sign.
    native_nodiscard friend native_inline native_const vec operator-(vec a) { return vec(_mm_xor_ps(a.value, _mm_set1_ps(-0.f))); }
    /// Return a mask whose lanes are true where `a < b` holds. NaN lanes yield false.
    native_nodiscard friend native_inline native_const mask_type operator<(vec a, vec b) {
    ...
    native_artificial native_nodiscard friend native_inline native_pure vec fma(vec a, vec b, vec c) { return vec(_mm_fmadd_ps(a.value, b.value, c.value)); }
    /// Compute the native square root in every lane.
    native_artificial native_nodiscard friend native_inline native_pure vec sqrt(vec a) { return vec(_mm_sqrt_ps(a.value)); }
    /// Round to an integral value, ties to even, independent of ambient direction.
    native_artificial native_nodiscard friend native_inline native_const vec round_even(vec a) { return vec(_mm_round_ps(a.value, _MM_FROUND_TO_NEAREST_INT | _MM_FROUND_NO_EXC)); }
    /// Construct normal powers of two; require integral exponents in [-126,127].
    native_nodiscard friend native_inline native_const vec normal_pow2(vec n) { return vec(_mm_castsi128_ps(_mm_slli_epi32(_mm_add_epi32(_mm_cvttps_epi32(n.value), _mm_set1_epi32(127)), 23))); }
    ...
    native_artificial native_nodiscard static native_inline native_const vec from_bits(bits_type bits) noexcept { return vec(_mm_castsi128_ps(bits.value)); }
    /// Reinterpret unsigned lane words as binary32, without normalization.
    ...
    native_nodiscard static native_inline native_pure vec loadu(native_noescape float const * p) { return load_memory<1>(p); }
    /// Synonym for an unaligned full-vector store.
    native_inline void storeu(native_noescape float * p) const { store_memory<1>(p); }
  ...
  /// \ingroup vectors
  /// Raw x86 float storage; the Arch argument fixes comparison-mask representation.
  template <::native::isa Arch> requires NATIVE_ARCH_REQUIRES(Arch) struct vec<float, 8,Arch> : detail::register_memory<vec<float,8,Arch>, 8> {
    static constexpr isa architecture=Arch;
    template <class T> using rebind = vec<T,8,Arch>;
    using vector_mask_type=vec<mask32,8,Arch>;
    using mask_type=std::conditional_t<bool(NATIVE_HAS_AVX512VL),predicate<8,Arch>,vec<mask32,8,Arch>>;
    using mask = mask_type;
    ...
    native_reinitializes native_inline constexpr vec & operator=(vec const &) = default;
    /// Broadcast x to all lanes.
    native_inline vec(float x) : value(_mm256_set1_ps(x)) {}
    /// Adopt native lane storage without numerical conversion.
    native_inline constexpr vec(__m256 x) : value(x) {}
    /// Load every logical lane; no extra alignment is required.
    native_nodiscard static native_inline native_pure vec load(native_noescape float const * p) { return load_memory<1>(p); }
    /// Store every logical lane; no extra alignment is required.
    native_inline void store(native_noescape float * p) const { store_memory<1>(p); }
    /// Add corresponding floating-point lanes using the caller's rounding and denormal environment.
    native_artificial native_nodiscard friend native_inline native_pure vec operator+(vec a, vec b) { return vec(_mm256_add_ps(a.value, b.value)); }
    /// Subtract corresponding floating-point lanes using the caller's rounding and denormal environment.
    native_artificial native_nodiscard friend native_inline native_pure vec operator-(vec a, vec b) { return vec(_mm256_sub_ps(a.value, b.value)); }
    /// Multiply corresponding floating-point lanes using the caller's rounding and denormal environment.
    native_artificial native_nodiscard friend native_inline native_pure vec operator*(vec a, vec b) { return vec(_mm256_mul_ps(a.value, b.value)); }
    /// Divide corresponding floating-point lanes using the caller's rounding and denormal environment.
    native_artificial native_nodiscard friend native_inline native_pure vec operator/(vec a, vec b) { return vec(_mm256_div_ps(a.value, b.value)); }
    /// Negate every logical lane; floating-point lanes change sign.
    native_nodiscard friend native_inline native_const vec operator-(vec a) { return vec(_mm256_xor_ps(a.value, _mm256_set1_ps(-0.f))); }
    /// Return a mask whose lanes are true where `a < b` holds. NaN lanes yield false.
    native_nodiscard friend native_inline native_const mask_type operator<(vec a, vec b) {
      if constexpr(bool(NATIVE_HAS_AVX512VL))
    ...
    /// Compute a*b+c with one fused rounding per lane.
    native_artificial native_nodiscard friend native_inline native_pure vec fma(vec a, vec b, vec c) { return vec(_mm256_fmadd_ps(a.value, b.value, c.value)); }
    /// Compute the native square root in every lane.
    native_artificial native_nodiscard friend native_inline native_pure vec sqrt(vec a) { return vec(_mm256_sqrt_ps(a.value)); }
    /// Round to an integral value, ties to even, independent of ambient direction.
    native_artificial native_nodiscard friend native_inline native_const vec round_even(vec a) { return vec(_mm256_round_ps(a.value, _MM_FROUND_TO_NEAREST_INT | _MM_FROUND_NO_EXC)); }
    /// Construct normal powers of two; require integral exponents in [-126,127].
    native_nodiscard friend native_inline native_const vec normal_pow2(vec n) { return vec(_mm256_castsi256_ps(_mm256_slli_epi32(_mm256_add_epi32(_mm256_cvttps_epi32(n.value), _mm256_set1_epi32(127)), 23))); }
    ...
    native_nodiscard native_inline native_pure bits_type to_bits() const noexcept { return bits(); }
    /// Reinterpret unsigned lane words as binary32, without normalization.
    native_artificial native_nodiscard static native_inline native_const vec from_bits(bits_type bits) noexcept { return vec(_mm256_castsi256_ps(bits.value)); }
    /// Reinterpret unsigned lane words as binary32, without normalization.
    native_nodiscard static native_inline native_const vec from_bits(std::uint32_t bits) noexcept { return from_bits(bits_type(bits)); }
    /// Broadcast one binary32 value to every lane.
    native_nodiscard static native_inline native_const vec from_float(float x) noexcept { return vec(x); }
    ...
    native_inline void store_bits_partial(native_noescape std::uint32_t * p,std::size_t n) const noexcept { bits().store_partial(p,n); }
    /// Load one lane from each array element, in array order.
    native_inline vec(std::array<float,8> const & values) noexcept : vec(loadu(values.data())) {}
#if defined(__clang__)
    /// Convert one argument per lane; exceptions follow those named-lvalue conversions.
    ...
    /// Convert one argument per lane; exceptions follow those named-lvalue conversions.
    template <class... X> requires (sizeof...(X)==8) && (std::convertible_to<X,float> && ...)
    native_inline vec(X... x) noexcept((noexcept(static_cast<float>(x)) && ...)) : vec(loadu(std::array<float,8>{static_cast<float>(x)...}.data())) {}
#endif
    /// Apply the corresponding lane-wise add operation in place and return *this.
    native_inline vec & operator+=(vec b) noexcept { return *this=*this+b; }
    /// Apply the corresponding lane-wise subtract operation in place and return *this.
