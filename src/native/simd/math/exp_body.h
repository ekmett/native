...
namespace NATIVE_BACKEND_NAMESPACE::native {
  ...
  native_nodiscard native_flatten native_inline native_pure std::array<V, N> exp(std::array<V, N> const & input) noexcept {
  ...
  native_nodiscard native_inline native_pure V exp(V x) noexcept {
    return ::NATIVE_BACKEND_NAMESPACE::native::exp<Flush>(std::array{x})[0];
  }
#if NATIVE_HAS_ARM_NEON
  native_nodiscard native_inline native_pure float32x4_t exp(float32x4_t x) noexcept {
    return ::NATIVE_BACKEND_NAMESPACE::native::exp(fp32x4(x)).value;
  }
  template<std::size_t N>
  native_nodiscard native_inline native_pure std::array<float32x4_t, N> exp(std::array<float32x4_t, N> const & input) noexcept {
