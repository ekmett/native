...
namespace native::detail::arm_bf16 {

  // Indexed four-element sources never read the upper register half. Undefined
  // shuffle lanes keep that unused half from requiring a duplicate/move.
  ...
  float32x2_t bfdot(float32x2_t acc, bfloat16x4_t a, bfloat16x4_t b) noexcept {
    ...
    auto source = ::native::detail::arm_register_order(b);
  ...
  float32x2_t bfdot_lane(float32x2_t acc, bfloat16x4_t a, bfloat16x4_t b) noexcept {
    acc = ::native::detail::arm_register_order(acc);
    a = ::native::detail::arm_register_order(a);
    auto source = ::native::detail::arm_register_order(__builtin_shufflevector(b, b, 0, 1, 2, 3, -1, -1, -1, -1));
