...
export namespace native {
  ...
  constexpr simd<std::uint8_t, 16, Arch> aese(simd<std::uint8_t, 16, Arch> state, simd<std::uint8_t, 16, Arch> round_key) noexcept {
    if consteval { return detail::arm_constant::aes_round<false>(state, round_key); }
    else {
  ...
  template<isa<arm> Arch> requires(!Arch.has(arm_feature::aes) && requires { sizeof(simd<std::uint8_t, 16, Arch>); })
  native_nodiscard consteval simd<std::uint8_t, 16, Arch> aese(simd<std::uint8_t, 16, Arch> state, simd<std::uint8_t, 16, Arch> round_key) noexcept {
    return detail::arm_constant::aes_round<false>(state, round_key);
  }
  ...
  constexpr simd<std::uint8_t, 16, Arch> aesd(simd<std::uint8_t, 16, Arch> state, simd<std::uint8_t, 16, Arch> round_key) noexcept {
    if consteval { return detail::arm_constant::aes_round<true>(state, round_key); }
    else {
  ...
  /// Evaluate constant operands without the instruction feature.
  template<isa<arm> Arch> requires(!Arch.has(arm_feature::aes) && requires { sizeof(simd<std::uint8_t, 16, Arch>); })
  native_nodiscard consteval simd<std::uint8_t, 16, Arch> aesd(simd<std::uint8_t, 16, Arch> state, simd<std::uint8_t, 16, Arch> round_key) noexcept {
    return detail::arm_constant::aes_round<true>(state, round_key);
  }
  ...
  constexpr simd<std::uint8_t, 16, Arch> aesmc(simd<std::uint8_t, 16, Arch> state) noexcept {
    if consteval { return detail::arm_constant::aes_mix<false>(state); }
    else {
  ...
  /// Evaluate constant operands without the instruction feature.
  template<isa<arm> Arch> requires(!Arch.has(arm_feature::aes) && requires { sizeof(simd<std::uint8_t, 16, Arch>); })
  native_nodiscard consteval simd<std::uint8_t, 16, Arch> aesmc(simd<std::uint8_t, 16, Arch> state) noexcept {
    return detail::arm_constant::aes_mix<false>(state);
  }
  ...
  constexpr simd<std::uint8_t, 16, Arch> aesimc(simd<std::uint8_t, 16, Arch> state) noexcept {
    if consteval { return detail::arm_constant::aes_mix<true>(state); }
    else {
  ...
  native_nodiscard consteval simd<std::uint8_t, 16, Arch> aesimc(simd<std::uint8_t, 16, Arch> state) noexcept {
    return detail::arm_constant::aes_mix<true>(state);
  }
