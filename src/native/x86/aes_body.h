...
export namespace native {
  ...
  constexpr simd<std::uint8_t, 16, Arch> aesenc(
    simd<std::uint8_t, 16, Arch> state, simd<std::uint8_t, 16, Arch> round_key) noexcept {
    if consteval {
      return detail::x86_aes_constant::round<false, false>(state, round_key);
    } else {
      return simd<std::uint8_t, 16, Arch>::from_native(
        detail::x86_aes::aesenc<Arch>(state.to_native(), round_key.to_native()));
    }
  }
  ...
  native_nodiscard consteval simd<std::uint8_t, 16, Arch> aesenc(
    simd<std::uint8_t, 16, Arch> state, simd<std::uint8_t, 16, Arch> round_key) noexcept {
    return detail::x86_aes_constant::round<false, false>(state, round_key);
  }
  ...
  template<isa<x86> Arch> requires(Arch.has(x86_feature::aes))
  native_nodiscard native_inline native_const native_target("aes")
  constexpr simd<std::uint8_t, 16, Arch> aesenclast(
    simd<std::uint8_t, 16, Arch> state, simd<std::uint8_t, 16, Arch> round_key) noexcept {
    if consteval {
      return detail::x86_aes_constant::round<false, true>(state, round_key);
    } else {
      return simd<std::uint8_t, 16, Arch>::from_native(
        detail::x86_aes::aesenclast<Arch>(state.to_native(), round_key.to_native()));
    }
  }
  ...
  template<isa<x86> Arch> requires(!Arch.has(x86_feature::aes) &&
      requires { sizeof(simd<std::uint8_t, 16, Arch>); })
  native_nodiscard consteval simd<std::uint8_t, 16, Arch> aesenclast(
    simd<std::uint8_t, 16, Arch> state, simd<std::uint8_t, 16, Arch> round_key) noexcept {
    return detail::x86_aes_constant::round<false, true>(state, round_key);
  }
  ...
  template<isa<x86> Arch> requires(Arch.has(x86_feature::aes))
  native_nodiscard native_inline native_const native_target("aes")
  constexpr simd<std::uint8_t, 16, Arch> aesdec(
    simd<std::uint8_t, 16, Arch> state, simd<std::uint8_t, 16, Arch> round_key) noexcept {
    if consteval {
      return detail::x86_aes_constant::round<true, false>(state, round_key);
    } else {
      return simd<std::uint8_t, 16, Arch>::from_native(
        detail::x86_aes::aesdec<Arch>(state.to_native(), round_key.to_native()));
    }
  }
  ...
  template<isa<x86> Arch> requires(!Arch.has(x86_feature::aes) &&
      requires { sizeof(simd<std::uint8_t, 16, Arch>); })
  native_nodiscard consteval simd<std::uint8_t, 16, Arch> aesdec(
    simd<std::uint8_t, 16, Arch> state, simd<std::uint8_t, 16, Arch> round_key) noexcept {
    return detail::x86_aes_constant::round<true, false>(state, round_key);
  }
  ...
  template<isa<x86> Arch> requires(Arch.has(x86_feature::aes))
  native_nodiscard native_inline native_const native_target("aes")
  constexpr simd<std::uint8_t, 16, Arch> aesdeclast(
    simd<std::uint8_t, 16, Arch> state, simd<std::uint8_t, 16, Arch> round_key) noexcept {
    if consteval {
      return detail::x86_aes_constant::round<true, true>(state, round_key);
    } else {
      return simd<std::uint8_t, 16, Arch>::from_native(
        detail::x86_aes::aesdeclast<Arch>(state.to_native(), round_key.to_native()));
    }
  }
  ...
  template<isa<x86> Arch> requires(!Arch.has(x86_feature::aes) &&
      requires { sizeof(simd<std::uint8_t, 16, Arch>); })
  native_nodiscard consteval simd<std::uint8_t, 16, Arch> aesdeclast(
    simd<std::uint8_t, 16, Arch> state, simd<std::uint8_t, 16, Arch> round_key) noexcept {
    return detail::x86_aes_constant::round<true, true>(state, round_key);
  }
  ...
  /// Substitute the odd words and return their substituted and rotated forms
  ...
  constexpr simd<std::uint8_t, 16, Arch> aeskeygenassist(simd<std::uint8_t, 16, Arch> state) noexcept {
    if consteval {
      ...
      return simd<std::uint8_t, 16, Arch>::from_native(
        detail::x86_aes::aeskeygenassist<Arch, Imm8>(state.to_native()));
    }
  }
