...
// Intentionally repeatable under one target family.
  public:
    ...
    SIMD_WIDE_TARGET simd_inline constexpr wide()
      ...
      requires (detail::wide_target<T> == SIMD_WIDE_INDEX) && detail::wide_target_default<T,N>
      : registers{} {}
    ...
    SIMD_WIDE_TARGET simd_inline constexpr wide(std::array<T, N> values)
      ...
      requires (detail::wide_target<T> == SIMD_WIDE_INDEX)
      : registers(std::move(values)) {}
    /// Construct one element from each argument, preserving argument order.
    template<class... U> requires (detail::wide_target<T,U...> == SIMD_WIDE_INDEX) && (sizeof...(U) == N && N != 0) &&
    ...
    template<class U> requires (detail::wide_target<T,U> == SIMD_WIDE_INDEX) && (N != 1) && (!detail::wide_traits<std::remove_cvref_t<U>>::value) &&
      (!std::same_as<std::remove_cvref_t<U>,std::array<T, N>>) &&
    ...
    template<class U> requires (detail::wide_target<T,U> == SIMD_WIDE_INDEX) && (!std::same_as<T,U>) && std::constructible_from<T,U const &>
    ...
    /// Convert each element of a same-size pack; explicitness follows the element conversion.
    template<class U> requires (detail::wide_target<T,U> == SIMD_WIDE_INDEX) && (!std::same_as<T,U>) && std::constructible_from<T,U &&>
    ...
    simd_nodiscard static SIMD_WIDE_TARGET simd_inline constexpr wide broadcast(T const & value)
      ...
      requires (detail::wide_target<T> == SIMD_WIDE_INDEX) && (N == 0 || std::copy_constructible<T>) {
      return wide(value, std::make_index_sequence<N>{});
    }

    /// Invoke `f` with all elements as arguments; the empty pack calls `f()`.
    template<class F> requires (detail::wide_target<T> == SIMD_WIDE_INDEX)
  ...
  private:
    template<class W, std::size_t... I> requires (detail::wide_target<T,W> == SIMD_WIDE_INDEX)
    ...
    template<class U, std::size_t... I> requires (detail::wide_target<T> == SIMD_WIDE_INDEX) && std::same_as<std::remove_cvref_t<U>,T>
    ...
    template<class F, class A, std::size_t ... I> requires (detail::wide_target<A> == SIMD_WIDE_INDEX)
